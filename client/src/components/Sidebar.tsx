...
const Sidebar = ({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (val: boolean)=> boolean}) => {
  ...
  return (
    ...
    flex flex-col h-full transform transition-transform-transform duration-200 ease-in-out
    md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>

      {/* Logo */}
      <div className="p-6 pb-4">
        <div className = "text-xl tracking-tight text-slate-800 flex items items-center gap-1.5">
      ...
      {navItems.map((item)=>{
        ...
        return (
          ...
          className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm
            transition-all duration-150 border ${isActive ? "bg-red-50 text-red-600 border-red-100 " :
