          ...
          <form action="" className="space-y-5" onSubmit={handleSchedule}>
            ...
            <div>
              ...
              className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl
               text-slate-900 text-sm placeholder-slate-400 outline-none resie-none" 
            ...
            <div>
              ...
              {mediaFile ? (
                ...
                border-2 border-dashed border-slate-200 rounded0xl cursor-pointer
                hover:border-red-300 hover:bg-red-50/30 transition-all group" > 

                  <span className="text-sm text-slate-500 group-hover:text-red-600transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="date" required className="w-full pl-10 pr-4 py-2.5bg-slate-50 border-slate-200 rounded-lg text-slate-900 text-sm outline-none" 
                ...
                <div className="relative">
                  ...
                  <input type="time" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border-slate-200 
                  rounded-lg text-slate-900 text-sm outline-none" 
            ...
            justify-center gap-2 py-3.5 bg-red-500 hover:bg-red-600 transition-all text-white rounded-lg">
              {loading ? (
                <>
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
              ...
              <h3 className="text-slate-900 text-sm font-sm">Upcoming</h3>
            
            <span className="ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">{scheduled.length}</span>
          </div>
          
          {/* Platform images inside the box */}
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
            {scheduled.length === 0 ? (
              <div className="py-10 text-center text-slate-400 text-sm ">No posts scheduled yet</div>)
              :(
                scheduled.map((post)=>(
                  <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors border-b border-slate-50 last:border-b-0">
                    <div className="flex items-start gap-3 mb-2">
                      {/* Platform Icons */}
                      <div className="flex gap-1.5">
                        {post.platforms.map((pl:string)=>{
                          const meta = PLATFORMS.find((p)=> p.id === pl);
                          return meta ? <meta.icon key={pl} className="size-4 text-slate-600"/>: null
                        })}
                      </div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">
                          {post.mediaType}</span>
                        }
                        <span className="text-xs text-slate-400 whitespace-nowrap">{new  Date(post.scheduledFor).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))
      ...
      {/* published */}
       <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
              ...
              <h3 className="text-slate-900 text-sm font-sm">Published</h3>
