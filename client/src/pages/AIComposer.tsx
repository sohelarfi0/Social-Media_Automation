...
const AIComposer = () => {
  ...
  const fetchGenerations = async ()=>{
    try {
      const {data} = await api.get("api/posts/genrations")
      setGenerations(data)
  ...
  const handleGenerate = async ()=> {
    ...
    try {
      ...
      toast.success("Content genrated!")
    } catch (error:any) {
  ...
  return (
    <div className='max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700'>
      ...
      <div className='space-y-6 text-center mt-20'>
        ...
        <div className='relative group mt-2'>
          ...
          <div className='absolute bottom-4 right-2.5 flex items-center gap-3 text-sm'>
            ...
            className= ' bg-slate-900 hover:bg-slate-700 text-white flex items-center gap-2 px-4 py-2 rounded-lg'>
              {loading ? (
                <>
...

          {
            generations.length === 0 && (
              <div className='col-span-full py-20 text-center space-y-2'>
                ...
                <p className='text-slate-400 text-sm'>No content generated yet. Try generating some content using theAI.</p>
              </div>
            )
          }
        </div>
      ...
      {/* Scheduler Modal */}
      {activeScheduler && (
        <div className='fixed inset-0 minh-screen z-50 flex items-center justify-center p-4
        bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300'>
          ...
          border-slate-100 overflow-hidden flex flex-col max-vh-[90vh]'>
              ...
              <div className='space-y-6'>
                <div>
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {PLATFORMS.map((p)=>{
                      const active = selectdPlatforms.includes(p.id);
                      return(
                        <button key={p.id} onClick={()=> 
                          setSelectedPlatforms((prev)=>(prev.includes(p.id) ? 
                        prev.filter((x)=> x !== p.id) : [...prev, p.id]))}
                        className={`p-2.5 rounded-md border text-xs ${active ? "bg-red-500/80 text-white" :
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                  <div className='relative'>
                    <CalendarIcon className='size-4 absoute left-4 top-1/2 
                    -translate-y-0.5 text-slate-400' />
                  ...
                  <div className='relative'>
                    <ClockIcon className='size-4 absoute left-4 top-1/2 
                    -translate-y-0.5 text-slate-400' />
